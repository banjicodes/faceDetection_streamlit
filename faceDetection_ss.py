...
st.title("Deep Learning based Face Detection using Caffe Model in OpenCV")
img_file_buffer = st.file_uploader("Please choose a file", type=['jpg', 'jpeg', 'png'])
...
if img_file_buffer is not None:
    ...
    conf_threshold = st.slider("Adjust Confidence Threshold", min_value=0.0, max_value=1.0, step=.01, value=0.5)
    ...
    if file_name != st.session_state.uploaded_file_name:
        # Set the uploaded_file_name equal to the name of the file that was just uploaded.
        ...
        st.write("You uploaded a new image, calling the face detection model.")
    else:
        st.write("Oh nice! same image used, processing with the previous detections.")
